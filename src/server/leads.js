...
router.get("/", async (req, res) => {
  try {
    ...
    if (
      ...
      throw new Error(
        "BatchData API token not properly configured. Please set BATCHDATA_API_TOKEN in .env file."
      );
    }
    ...
    // if (street)
    searchData.searchCriteria.compAddress.street = "41028 N Congressional Dr";
    if (city) searchData.searchCriteria.compAddress.city = city;
    if (state)
      searchData.searchCriteria.compAddress.state = state.toUpperCase();
    // if (zip)
    searchData.searchCriteria.compAddress.zip = "85086";
    ...
    console.log("BatchData API response:", {
      ...
      properties: response?.data?.data?.results?.properties,
    });
    ...
    const formattedLeads = leads.map((property) => ({
      ...
      owner_mailing_street:
        property.mailing_address || property.mailingAddress || "",
      owner_mailing_city: property.mailing_city || property.mailingCity || "",
      owner_mailing_state:
        property.mailing_state || property.mailingState || "",
    ...
    if (error.response?.status === 401) {
      errorMessage =
        "Invalid BatchData API token. Please check your configuration.";
      statusCode = 401;
    } else if (error.response?.status === 404) {
      errorMessage =
        "BatchData API endpoint not found. Please check the API URL.";
      statusCode = 404;
    } else if (error.response?.status === 429) {
      errorMessage =
        "BatchData API rate limit exceeded. Please try again later.";
      statusCode = 429;
    }
